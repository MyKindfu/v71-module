...
import { postArcImg, ArcImgSaveOpts } from '../arc'
...
/** 保存选中的图片 */
export function saveCommonScan(
  ...
  const commonScanImgBox = <HTMLDivElement> dialogCtx.querySelector(uiOpts.commonScanImgBoxSelector)

  const imgs = commonScanImgBox.querySelectorAll<HTMLImageElement>('img.check-true')
  ...
  const arcPkgType = <HTMLSelectElement> dialogCtx.querySelector(uiOpts.arcPkgTypeSelctor)
 ...
 /*  imgs.forEach((val, idx, array) => {
  ...
  // 将遍历到的图片保存到服务器
  const ret$ = from(imgs).pipe(
    mergeMap(param => saveArcImg2(dialogCtx, uiOpts, pdata, param, param.src), 1),
    tap(data => {
      info(['图片保存成功', data])
    }),
    mapTo(null),
    reduce((acc: number, curr: null) => {
      acc = acc + 1
      return acc
    }, 0),
    tap(data => {
...
export function saveArcImg2(
  ...
  const ret$ = postArcImg(data, imgUrl).pipe(
    tap(() => {

      // 删除已经成功的图片，避免后续发送服务器出错，二次保存问题
      const commonScanList = <HTMLLabelElement> imgEle.closest('.common-scan-list')
      ...
      const imgs = commonScanImgBox.querySelectorAll<HTMLImageElement>('img.common-scan-img')
...
/** 开始扫描并打开窗口 */
export function openCommonScan(dialogCtx: HTMLDivElement, uiOpts: UIOpts, arcCode: string): void {

  const commonScanBtn = <HTMLDivElement> dialogCtx.querySelector(uiOpts.commonScanSelector)
...
/** 监控公共扫描websocket状态 */
export function keepCommonScanWebsocket(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  commonScanWs: any,
): void {
  ...
  setcommonScanBtnAvailable(dialogCtx, uiOpts, false) // 公共扫描按钮禁用
    ...
    // 获取随机数uuid用于扫描进程
  const scan_uuid: string = UUID.generate()

  if (commonScanWs) {

    commonScanWs.onopen = (evt: any) => {
      log('公共扫描服务连接已启动 ...')
    ...
    commonScanWs.onmessage = (evt: any) => {
      log('返回扫描服务结果!')
      ...
      if (evt && evt.data) {
        ...
        if (ws_data && ws_data.wrkimgs.length > 0 && ws_data.action === 'doscan') {
          ...
          $(<HTMLUListElement> dialogCtx.querySelector(uiOpts.commonScanContainerSelector)).removeClass('hidden')
...
/** 根据扫描结果获取扫描图片 */
export function getCommonScanImg(
  commonScanWs: any,
  wrkimgs: string[],
  wrkno: string,
...
/** 展示扫描图片 */
export function showCommonScanArcImg(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  commonScanWs: any,
  img64: string,
...
/** 选择图片 */
export function checkCommonScanImg(element: HTMLElement) {
  const warp = <HTMLDivElement> element.closest('.common-scan-list')
  const checkMark = <HTMLElement> warp.querySelector('.check-mark')
  const imgContent = <HTMLElement> warp.querySelector('.common-scan-img-content')
  if (checkMark) {
...
/** 全选 */
export function checkSquareCommonScanImg(
  dialogCtx: HTMLDivElement,
): void {

  dialogCtx.querySelectorAll<HTMLDivElement>('.common-scan-list')
    .forEach(element => {
      const checkMark = <HTMLElement> element.querySelector('.check-mark')
      const checkImg = <HTMLElement> element.querySelector('.common-scan-img')
      const imgContent = <HTMLElement> element.querySelector('.common-scan-img-content')
      if (checkMark) {
...
/** 取消公共扫描窗口 */
export function commonScanCancel(dialogCtx: HTMLDivElement, uiOpts: UIOpts): void {
...
/** 关闭公共扫描窗口 */
export function commonScanClose(dialogCtx: HTMLDivElement, uiOpts: UIOpts): void {
  const box = <HTMLDivElement> dialogCtx.querySelector(uiOpts.commonScanContainerSelector)
...
/** 旋转公共扫描图片 */
export function rotateImgCommonScan(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  rotateImgBtn: HTMLElement,
  angle: number,
): Observable<null> {
  ...
  switch (rotateImgBtn.nodeName) {
    case 'LABEL':
      ...
      break
    default: {
      const label = <HTMLLabelElement> rotateImgBtn.closest('label' + uiOpts.commonScanSelector)
      if (label) {
        rotateImgBtn.classList.add('disabled')
      }
      break
  ...
  // 将遍历到的图片进行旋转
  const ret$ = from(imgs).pipe(
    mergeMap(param => rotateBase64Img(param, angle), 5),
    tap(data => {
      info(['图片旋转成功', data])
    }),
    mapTo(null),
...
/** 旋转base64图片 */
export function rotateBase64Img(
  imgEle: HTMLImageElement,
  edg: number,
): Observable<null> {
