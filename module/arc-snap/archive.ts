...
import {
  genArcPkgOptionsFromArcCodeContent,
  // getArccodeByXM,
  // getArcImg,
  getHouseImgByXM,
  initArcCombined,
 ...
 */
export function initArcCode(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  data: ArcCodeInitOpts,
) {
  ...
  // 两步自动初始化
  const ret$ = initArcCombined(data).pipe(
    mergeMap(ret => {
      const opts = genArcPkgOptionsFromArcCodeContent(ret.arcPkgContents)
      return updateArcPkgTypeSelect(dialogCtx, uiOpts, opts).pipe(
        mapTo(ret),
      )
    }),
    // tap(({ arcPkgContents }) => {
...
/** 读取本地图片文件输出为 dataURL */
export function handleFiles(files: FileList, maxPixel: number): Observable<ImgFileInfo> {
  const imageTypeRegex = /^image\//
  const file$ = ofrom(files).pipe(
    filter(file => imageTypeRegex.test(file.type)),
  )
  const img = new Image()
  const load$ = fromEvent<MouseEvent>(img, 'load').pipe(
    pluck<MouseEvent, HTMLImageElement>('target'),
    take(files.length),
  )

  const ret$ = file$.pipe(
    concatMap(file => {
      const proc$ = load$.pipe(
        take(1),
        mergeMap((target: HTMLImageElement) => {
          ...
          return takeThumbnail(target, sopts).pipe(
            tap(() => {
      ...
      return proc$.pipe(
        map(url => {
          ...
          return { ...info, url, size: url.length }
        }),
        timeout(15000),
      )
    }),
  ...
  return ret$.pipe(
    reduce((acc: ImgFileInfo[], curr: ImgFileInfo) => {
      acc.push(curr)
      return acc
    }, []),
...
/** 显示指定职工的房管局来源共享图片 */
export function showHouseImgByXM(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  data: HouseOpts,
): Observable<null> {

  return getHouseImgByXM(data).pipe(
    tap(arr => {
      showArcImg(dialogCtx, uiOpts, arr)
    }),
    mapTo(null),
