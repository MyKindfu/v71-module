...
export function initRxCam(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  initialOpts: InitialOpts,
): Observable<RxCam> {
  const opts: InitialOpts = { ...initialCamOpts, ...initialOpts }
  const cam$ = RxCamFactory(opts)

  const ret$ = cam$.pipe(
    mergeMap(cam => {
      return cam.connect().pipe(
        tap(constraints => {
          // subject.next({
...
export function takePhoto(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  cam: RxCam, options?: Partial<SnapOpts>,
) {
...
export function toggleCam(cam: RxCam, sidx?: StreamIdx): Observable<MediaStreamConstraints> {
...
export function genCaptureOpts(
  dialogCtx: HTMLDivElement,
  uiOpts: UIOpts,
  options?: Partial<SnapOpts>,
): SnapOpts {

  const sidx = getCurStreamIdx(dialogCtx, uiOpts)
  const rotate = getLastCamRotateValue(sidx)
  const opts = <SnapOpts> { rotate, ...options }

  return opts
}

/** 根据配置参数、采集参数生成摄像头对应的采集旋转角度列表 htmlelement */
export function genCamsRotateList(count: number, options: InitialOpts): void {
...
export function getLastCamRotateValue(sidx: StreamIdx): number {
...
export function updateCamRotateValue(sidx: StreamIdx, rotate: number): void {
