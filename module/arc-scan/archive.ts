...
import { from as ofrom, of, EMPTY, Observable } from 'rxjs'
...
import { initialArcConfigs, UrlList } from './config'
import {
  ArcScanImgCombineSaveOpts,
  BankRegId,
  ImgPostParams,
  OcrRetObjectExt,
  QueryOcrMatchRet,
...
export function queryOcrRetMatch(
  sn: string,
  data: Map<Filename, OcrRetObject>,
  failedMap: Map<Filename, string>,
  processingSet: Set<Filename>,
): Observable<QueryOcrMatchRetRow> {

  if (!sn) {
    throw new Error('流水号sn不能空')
  }
  const arr = <OcrRetObjectExt[]> []
  const tmpMap = <Map<number, OcrRetObjectExt>> new Map()
  let id = 1
  ...
  for (const row of data.values()) {
    ...
    const tmpRow = <OcrRetObjectExt> { ...row }
    tmpRow.id = id + ''
...
export function combineSaveScanImg(data: ArcScanImgCombineSaveOpts): Observable<boolean> {
  // creater: string
  // pic: string  // 图片 base64
  // DZDID: number // 对账单ID

  // 'mugShotController/saveYhjsxxFile.do'
  const req$ = post(UrlList.combineSaveScanImg, { data })
...
function handleQueryOcrRetMatch(
  res: QueryOcrMatchRet | void,
  ...
  if (! res || !res.dat || !Array.isArray(res.dat)) {
    ...
    const singleRowsTmp = <QueryOcrMatchRetRow[]> [] // 唯一匹配的(可能流水号有差异)
    const singleRows2 = <QueryOcrMatchRetRow[]> [] // 唯一匹配的(可能流水号有差异)
    const multiRows = <QueryOcrMatchRetRow[]> []  // 多匹配的
    ...
    // 判断流水号是否相符
    const validRows = <QueryOcrMatchRetRow[]> []
    ...
    const combined$ = single$.pipe(
      merge(multi$),
    )

    const ret$ = <Observable<QueryOcrMatchRetRow>> valid$.pipe(
      merge(combined$),
      filter(row => !! row && row.DZDID.length > 0 ? true : false),
      map(row => {
        if (row) {
...
function filterQueryRetRow(data: QueryOcrMatchRetRow[]): {
  single: Set<string>, multi: Set<string>,
} {
  const single = <Set<string>> new Set() // row.ID
  const multi = <Set<string>> new Set()  // row.ID
...
function retrieveRowsFromQueryOcrMatchRetRows(data: QueryOcrMatchRetRow[], id: string): QueryOcrMatchRetRow[] {
  const ret = <QueryOcrMatchRetRow[]> []
...
export function saveSplitImg(data: ImgPostParams): Observable<void> {
  const url = data.url
 ...
 */
export function genSBIndex(name: Filename): number {
  if (! name) {
 ...
 */
export function notifyStartServerOcrTask(host: string) {
  if (! host) {
 ...
 */
export function notifyOcrServiceToStart(host: string): Observable<void> {
  // @ts-ignore
  // 本核心系统 URL，用于远程服务调用核心系统接口取数
  const selfUrl = <string> ST.base
