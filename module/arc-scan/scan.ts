...
import {
  setElmsValidState, updateScannedPageCount,
} from './ui'



export function initScanner(sym: symbol, scanOptions?: ScanOpts, wsOptions?: WsOpts): Scanner {
  const scanner = init(scanOptions, wsOptions)

  scanner.connect().pipe(
    tap(() => {
      scannerInstMap.set(sym, scanner)
      setElmsValidState(true)

      subject.next({
        action: Actions.scannerReady,
        payload: { sym },
      })
    }),
  )
...
export function getScannerInstbySym(sym: symbol): Scanner | void {
  return scannerInstMap.get(sym)
}


/** 开始扫描 返回图片文件路径数组 */
export function startScan(scanner: Scanner, clearAll: boolean = false): Observable<string[]> {
  ...
  const clearAndScan$ = scanner.clearAll().pipe(
    tap(() => {
  ...
  return tips$.pipe(
      withLatestFrom(clearAll ? clearAndScan$ : scan$),
      map(vals => vals[1]),
      take(1),  // 扫描结束返回文件列表后即取消 tips$ 等所有订阅
      tap(fileListArr => {
