...
import { init, Zkf } from './archive'
import { LoginDat, SrvEvent, WsResults } from './model'

export function factoryZkf(el: HTMLDivElement): Observable<Zkf> {
  const zkf: Zkf = init()
  zkf.connect()
  return subscribeEvent(zkf, el)
}

function subscribeEvent(zkf: Zkf, el: HTMLDivElement): Observable<Zkf> {
  return bindClickEvent(document).pipe(
    mergeMap(elm => {
      if (elm.matches('#BtnZKFCERT')) { // 采集
        return registerIntercept(zkf).pipe(
          mergeMap(() => {
            return zkf.register().pipe(
              map(res => res.ret),
              filter(ret => ret === 0 ? true : false),
              tap(() => initModal(el)),
              tap(() => {
                // @ts-ignore
                $(el).modal('show')
        ...
        return validateUser(el).pipe(
          filter(val => val !== '' ? true : false),
          mergeMap(() => registerIntercept(zkf)),
          tap(() => {
            const fingerImgbox = <HTMLDivElement> el.querySelector('.finger-img-box')
...
/** 登录检查用户名 */
function validateUser(el: HTMLDivElement): Observable<string> {

  const userName = <HTMLInputElement> el.querySelector('#zkfuname')

  return of(userName.value).pipe(
  tap(val => {
    if (!val) {
  ...
  filter(val => val ? true : false),
  mergeMap(code => checkUser(code)),
  )
}

function checkUser(code: string): Observable <string> {
...
/** 采集出错重新采集 */
function collectErr(zkf: Zkf): Observable<Zkf> {
  return zkf.register().pipe(
    mapTo(zkf),
  )
}
...
/** 出错采集页面还原 */
function initModal(el: HTMLDivElement) {
  const $cjcs = $(<HTMLElement> el.querySelector('#cjcs'))
  const img = <HTMLImageElement> el.querySelector('.zkfimg-box > img')
  const $btnSave = $(<HTMLButtonElement> el.querySelector('#BtnSaveZkf'))
...
/** 更新采集UI */
function updateUI(el: HTMLDivElement, dat: WsResults) {
  const $cjcs = $(<HTMLElement> el.querySelector('#cjcs'))
  const finger = <HTMLInputElement> el.querySelector('#zkfinger-tmp')
  const img = <HTMLImageElement> el.querySelector('.zkfimg-box > img')
  const $btnSave = $(<HTMLButtonElement> el.querySelector('#BtnSaveZkf'))
...
/** 采集事件 */
function onenroll(zkf: Zkf, dat: WsResults, el: HTMLDivElement): Observable<Zkf> {
  const data$ = of(dat)
  ...
  if (dat.ret !== 0) {
    ...
    return data$.pipe(
      ...
      // finalize(() => closeDev(zkf)),
    )
  }
}
/** 登录事件 */
function oncapture(
  zkf: Zkf, dat: WsResults, el: HTMLDivElement,
): Observable<Zkf> {
  if (dat.ret !== 0) {
    return of(dat).pipe(
      tap(() => {
...
function afterLogin(el: HTMLDivElement, dat: LoginDat) {
  const contentBox = <HTMLDivElement> el.querySelector('.finger-content')
  const fingerImgBox = <HTMLDivElement> el.querySelector('.finger-img-box')
...
/** 登录请求 */
function zkfLogin(el: HTMLDivElement, dat: WsResults): Observable<null> {
  if (dat.datatype && dat.datatype === 'template' && dat.data.template) {
    const nativetmp = <string> dat.data.template
    const user = <HTMLInputElement> el.querySelector('#zkfuname')
    ...
    .pipe(
      ...
      filter(res => res.state && res.state === 9 ? true : false),
      tap(res => {
        if (res.dat) {
...
/** 关闭设备 */
export function closeDev(zkf: Zkf): Observable<Zkf> {
  return zkf.closeDev().pipe(
   mapTo(zkf),
  )
}

/** 打开设备及检查 */
export function registerIntercept(zkf: Zkf) {
  return zkf.getStatus().pipe(
    map(res => res.ret),
    filter(ret => ret === 0 || ret === -10014 ? true : false),
    mergeMap(ret => {
      if (ret === 0) {
        return of(true)
      }
      else {
        return zkf.openDev().pipe(
          map(res => res.ret),
          tap(oret => {
            if (oret !== 0) {
...
// 绑定模态框内指定元素点击事件流
export function bindClickEvent(ctx: Document) {
  return fromEvent<MouseEvent>(ctx, 'click')
    .pipe(
      debounceTime(50),
      pluck<MouseEvent, HTMLButtonElement>('target'),
      filter(eventFilter),
    )
}


function eventFilter(elm: HTMLButtonElement): boolean {
  if (! elm || ! elm.nodeName) {
    return false
  }
  // @ts-ignore
