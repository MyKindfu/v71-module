import { init, CertKinds } from 'bjca';
 ...
 * @param uiOpts
 */
export function initCA(uiOpts) {
    const ca = init();
    getLoginConf().pipe(filter(res => res === '2' ? true : false))
        .subscribe(() => {
        const form = document.querySelector(uiOpts.formSelector);
        const nameInput = form.querySelector(uiOpts.usernameInputSelector);
 ...
 * @description 只用于CA采集
 */
export function initCollectCA() {
    const ca = init();
    ca.connect();
    return ca;
}
function validateUIOpts(uiOpts) {
    const form = document.querySelector(uiOpts.formSelector);
    const nameInput = form.querySelector(uiOpts.usernameInputSelector);
    const certIdInput = form.querySelector(uiOpts.certIdInputSelector);
    const pwdInput = form.querySelector(uiOpts.pwdInputSelector);
    const btnLogin = form.querySelector(uiOpts.pwdInputSelector);
    if (!form) {
        alert('表单不存在');
...
function bindFormLoginClick(uiOpts) {
    const form = document.querySelector(uiOpts.formSelector);
    const btnLogin = form.querySelector(uiOpts.btnLoginSelector);
    fromEvent(btnLogin, 'click').pipe(tap(() => toggleElmValidState(btnLogin, false)), debounceTime(500), mapTo(true))
        // event$.pipe(
        //     filter(() => !! form.useCA),
...
function subscribeEvent(ca, uiOpts) {
    const form = document.querySelector(uiOpts.formSelector);
    form.useCA = 0;
...
function userNameByYYZS(YYZS) {
    ...
    return res$.pipe(tap(res => {
        if (res && res.err) {
            throw new Error(res.msg ? res.msg : res.dat);
        }
    }));
}
function onUsbkeyInsert(ca, uiOpts) {
    const conf$ = getLoginConf();
    const list$ = ca.getUserList();
    forkJoin(conf$, list$).pipe(tap(([config]) => {
        const form = document.querySelector(uiOpts.formSelector);
        const nameInput = form.querySelector(uiOpts.usernameInputSelector);
    ...
    }), map(([, list]) => list), filter(data => !!data && Array.isArray(data) && !!data.length), mergeMap(data => {
...
function onUsbkeyRemove(uiOpts) {
    const form = document.querySelector(uiOpts.formSelector);
    const nameInput = form.querySelector(uiOpts.usernameInputSelector);
    getLoginConf().subscribe(res => {
        if (res && res === '2') {
            toggleElmValidState(nameInput, false);
...
// 更新输入框
function updateInputs(uiOpts, userCode) {
    const form = document.querySelector(uiOpts.formSelector);
    const nameInput = form.querySelector(uiOpts.usernameInputSelector);
...
function validateCertExpiry(info) {
    ...
    const now = new Date();
    const start = new Date(+startStr.slice(0, 4), // YYYY
    +startStr.slice(4, 6) - 1, // MM
    +startStr.slice(6, 8), // DD
    +startStr.slice(8, 10), // HH
    +startStr.slice(10, 12), // mm
    +startStr.slice(12, 14));
    const end = new Date(+endStr.slice(0, 4), +endStr.slice(4, 6) - 1, +endStr.slice(6, 8), +endStr.slice(8, 10), +endStr.slice(10, 12), +endStr.slice(12, 14));
    if (now < start) {
...
// 切换输入框状态有效
function toggleElmValidState(elm, valid) {
    elm.disabled = !valid;
}
