...
export function getElmAccessState(map: ThrottleAcMapModel, elm: HTMLElement): ElmAccessState | void {
  return map.get(elm)
}
 ...
 * 设置元素最近访问时间
 */
export function updateAccessDate(map: ThrottleAcMapModel, elm: HTMLElement) {
  const state: ElmAccessState = {
    date: new Date(),
    // @ts-ignore
...
function cleanAccessDate(map: ThrottleAcMapModel, elm: HTMLElement) {
  map.delete(elm)
}
...
export function setElmDurationDisabled(
  map: ThrottleAcMapModel,
  elm: ThrottleElm,
  duration: number,
): void {
  ...
  // 若原本就禁用状态则不还原可用
  setTimeout((mp: ThrottleAcMapModel, dom: HTMLButtonElement) => {
 ...
 */
export function matchedSearchElm(ev: MouseEvent, matchingCtor: string[]): HTMLButtonElement | void {
  const elm = <HTMLElement | null> ev.target
 ...
 * 若元素未设置，则实用默认值
 */
export function retriveThrottleLimit(elm: HTMLElement, defaultLimit: number): number {
  const ret: number = elm.dataset
