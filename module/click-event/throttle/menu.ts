...
const ctors: string[] = throttleMatchingCtor.menuLink
const defaultLimit: number = defaultThrottleLimits.menuLink


/** 处理左侧页面菜单 */
export const handleMenuLink: Filter = ev => {
 ...
 */
function matchedElm(ev: MouseEvent, matchingCtor: string[]): HTMLLinkElement | void {
  const elm = <HTMLLinkElement | null> ev.target
